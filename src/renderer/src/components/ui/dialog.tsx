...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      style={{ backgroundColor: 'var(--card)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--foreground)', boxShadow: '0 24px 64px rgba(0,0,0,0.8)' }}
      {...props}
    >
      {children}
      <DialogPrimitive.Close
        className="absolute right-4 top-4 rounded-sm opacity-50 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description ref={ref} className={cn('text-sm', className)} style={{ color: 'rgba(255,255,255,0.45)' }} {...props} />
))
